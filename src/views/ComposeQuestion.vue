        ...
        <div class="w-full p-4">
            <div class="w-full flex">
                ...
                <div class="flex-1 pl-3 mt-[0.45rem]">
                    <textarea v-model="title" ref="textareaTitle" class="w-full text-xl font-bold outline-none" placeholder="Title"></textarea>
                    <Divider />
                    <textarea v-model="content" ref="textareaContent" class="w-full text-lg outline-none" placeholder="What's happening..."></textarea>
                </div>
            </div>
            <Divider />
...
const postNormal = async () => {
    ...
    try {
        ...
        formData.append("type", '2'); //normal type
...
onMounted(() => {
    ...
    textareaTitle.value.style.height = '1.75rem';
    textareaContent.value.style.height = '7rem';
    textareaTitle.value.addEventListener("input", heightHandler)
    textareaContent.value.addEventListener("input", heightHandler)
})

onBeforeUnmount(() => {
    textareaTitle.value.removeEventListener("input", heightHandler)
    textareaContent.value.removeEventListener("input", heightHandler)
})
